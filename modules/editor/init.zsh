...
# Set empty $key_info values to an invalid UTF-8 sequence to
#  induce silent bindkey failure.
...
function editor-info {
	...
	if zstyle -t ':e4czmod:module:prompt' managed; then
		...
		if [[ "$KEYMAP" == 'vicmd' ]]; then
			zstyle -s ':e4czmod:module:editor:info:keymap:alternate' format 'REPLY'
			editor_info[keymap]="$REPLY"
		else
			zstyle -s ':e4czmod:module:editor:info:keymap:primary' format 'REPLY'
			editor_info[keymap]="$REPLY"

			if [[ "$ZLE_STATE" == *overwrite* ]]; then
				zstyle -s ':e4czmod:module:editor:info:keymap:primary:overwrite' format 'REPLY'
				editor_info[overwrite]="$REPLY"
			else
				zstyle -s ':e4czmod:module:editor:info:keymap:primary:insert' format 'REPLY'
				editor_info[overwrite]="$REPLY"
			fi
		fi
...
function zle-reset-prompt {
	if zstyle -t ':e4czmod:module:editor' ps-context; then
...
# Moves to the first non-blank character then enters vi insert mode and
...
function expand-or-complete-with-indicator {
	...
	# This is included to work around a bug in zsh which shows up when
	#  interacting with multi-line prompts.
	if [[ -z "$indicator" ]]; then
...
# Toggle the comment character at the start of the line. This is meant
#  to work around a buggy implementation of pound-insert in zsh.
# This is currently only used for the emacs keys because vi-pound-insert
#  has been reported to work properly.
function pound-toggle {
	if [[ "$BUFFER" = '#'* ]]; then
		# Because of an oddity in how zsh handles the cursor when the
		#  buffer size changes, we need to make this check before we
		#  modify the buffer and let zsh handle moving the cursor back
		#  if it's past the end of the line.
...
# Toggle comment at the start of the line. Note that we use pound-toggle
#  which is similar to pount insert, but meant to work around some issues
#  that were being seen in iTerm.
...
#  undefined things. In emacs mode they just insert a tilde, but bind these
#  keys in the main keymap to a noop op so if there is no keybind in the users
#  mode it will fall back and do nothing.
