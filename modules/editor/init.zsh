...
function editor-info {
	...
	if zstyle -t ':zmodule:prompt' managed; then
		# Clean up previous $editor_info.
		unset editor_info
		typeset -gA editor_info

		if [[ "$KEYMAP" == 'vicmd' ]]; then
			zstyle -s ':zmodule:editor:info:keymap:alternate' format 'REPLY'
			editor_info[keymap]="$REPLY"
		else
			zstyle -s ':zmodule:editor:info:keymap:primary' format 'REPLY'
			editor_info[keymap]="$REPLY"

			if [[ "$ZLE_STATE" == *overwrite* ]]; then
				zstyle -s ':zmodule:editor:info:keymap:primary:overwrite' format 'REPLY'
				editor_info[overwrite]="$REPLY"
			else
				zstyle -s ':zmodule:editor:info:keymap:primary:insert' format 'REPLY'
				editor_info[overwrite]="$REPLY"
			fi
		fi
...
function zle-reset-prompt {
	if zstyle -t ':zmodule:editor' ps-context; then
...
function expand-or-complete-with-indicator {
	local indicator
	zstyle -s ':zmodule:editor:info:completing' format 'indicator'
...
for keymap in 'emacs' 'viins'; do
	...
	# Expand .... to ../..
	if zstyle -t ':zmodule:editor' dot-expansion; then
...
if [[ "$key_bindings" == (emacs|) ]]; then
	...
	print "zmodule:editor:invalid key bindings: $key_bindings" >&2
fi
